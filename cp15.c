...
static Boolean cp15prvCoprocRegXferFunc(struct ArmCpu* cpu, void* userData, Boolean two, Boolean read, UInt8 op1, UInt8 Rx, UInt8 CRn, UInt8 CRm, UInt8 op2){
	...
	switch(CRn){
		...
		case 0:		//ID codes
			...
			if(!read && CRm != 0) goto fail;//cannot write to ID codes register and CRm must be zero for this read
			if(!op2){					//main ID register
				
				val = CPUID_PXA255;
				goto success;
			}
			else if(op2){				//cache type register - we lie here
