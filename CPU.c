...
static ArmBankedRegs* cpuPrvModeToBankedRegsPtr(ArmCpu* cpu, UInt8 mode){
	...
	switch(mode){
		...
		default:
			cpu->emulErrF(cpu, "cpuPrvModeToBankedRegsPtr()");
			return NULL;
	}
}
...
static _INLINE_ UInt8 cpuPrvArmAdrMode_3(ArmCpu* cpu, UInt32 instr, UInt32* addBeforeP, UInt32* addWritebackP, Boolean wasT, Boolean specialPC){
	...
	Boolean L = (instr & 0x00100000UL) != 0;
	Boolean H = (instr & 0x00000020UL) != 0;
	Boolean S = (instr & 0x00000040UL) != 0;
...
static Err cpuPrvCycleArm(ArmCpu* cpu){
	...
	//fetch instruction
	{
		if(!icacheFetch(&cpu->ic, pc = cpu->regs[15], 4, privileged, &fsr, &instr)){
...
static Err cpuPrvCycleThumb(ArmCpu* cpu){
	...
	Boolean privileged, vB, specialPC = false;
	...
	pc = cpu->regs[15];
	if(!icacheFetch(&cpu->ic, pc, 2, privileged, &fsr, &instrT)){
